...
function DiscordTag({user, voice}) {
    ...
    switch (true) {
        ...
        case !! (voice && voice[0]?.members.length > 1): outputtedUsername = Common.intl.intl.formatToPlainString(Common.intl.t['4SM/RX'], { user1: (user.globalName || user.username || voice[0]?.members[1].username), user2: (voice[0]?.members[1].globalName || voice[0]?.members[1].globalName || voice[0]?.members[1].username) }); break;
