...
function PartyMemberListBuilder({activity, users}) {
    ...
    return (
        <div className={NowPlayingClasses.partyList}>
            {playerFill.splice(0, 10).map((player, index, players) =>
                {
                    switch(player) {
                        case "anon": return <Common.Mask className={NowPlayingClasses.maskStack} mask={index !== players.length - 1 ? Common.Mask.Masks.VOICE_USER_SUMMARY_ITEM : null} width={16} height={16}>
                            <div className={NowPlayingClasses.emptyUser}>
                        ...
                        case null: return <Common.Mask className={NowPlayingClasses.maskStack} mask={index !== players.length - 1 ? Common.Mask.Masks.VOICE_USER_SUMMARY_ITEM : null} width={16} height={16}>
                            <div className={NowPlayingClasses.emptyUser} />
                        </Common.Mask>
                        default: return <Tooltip note={Common.UsernameUtils.getName(player)}>
                            <div>
                                <Common.Mask className={NowPlayingClasses.maskStack} mask={index !== players.length - 1 ? Common.Mask.Masks.VOICE_USER_SUMMARY_ITEM : null} width={16} height={16}>
    ...
    return (
        <>
            {activityProperties.platform !== "YT_MUSIC" && <Common.Flex align={Common.Flex.Align.END} className={NowPlayingClasses.actionsActivityFeed} direction={Common.Flex.Direction.VERTICAL} justify={Common.Flex.Justify.CENTER} style={{ flex: "0 1 auto", marginLeft: "20px" }}>
                {v2Enabled && isJoinable && activity?.party && activity?.party?.size ? null : <ActivityButtons user={user} activity={activity} onAction={action} />}
            </Common.Flex>}
        </>
    )
}
    ...
    return (
        <>
            ...
            <div className={NowPlayingClasses.partyStatusWrapper}>
                ...
                {isJoinable && <ActivityButtons user={user} activity={activity} onAction={action} />}
            </div>
        </>
    )
}