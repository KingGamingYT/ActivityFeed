...
class ActivityFeedSettingsCoachmarkStore extends betterdiscord.Utils.Store {
...
const ActivityFeedSettingsCoachmarkStore$1 = new ActivityFeedSettingsCoachmarkStore();
...
const settings = {
	...
	debug: {
		...
		resetCoachmark: {
			...
			innerText: "Reset",
			type: "button",
			onClick: () => ActivityFeedSettingsCoachmarkStore$1.setHasDismissedSettingsCoachmark(false)
		},
...
class GameNewsStore extends betterdiscord.Utils.Store {
	...
	async #fetchSubnauticaFeeds(application) {
		const rssFeed = await Promise.all([betterdiscord.Net.fetch(`https://unknownworlds-strapi.live.kraftonamericas.com/api/articles?sort[0]=published_date%3Adesc&sort[1]=id%3Adesc&sort[2]=published_date%3Adesc&start=0&limit=4`).then((r) => r.ok ? r.json() : null)]);
...
function FeedPopout({ application, gameId, articleUrl, close }) {
	...
	if (isNaN(application.id)) {
		return BdApi.React.createElement(betterdiscord.ContextMenu.Menu, { navId: "feed-overflow", onClose: close ?? ((e) => Common.FluxDispatcher.dispatch({ type: "CONTEXT_MENU_CLOSE" }).finally(e)) }, BdApi.React.createElement(betterdiscord.ContextMenu.Item, { id: "copy-article-link", label: "Copy Article Link", action: () => Common.Clipboard(articleUrl) }), !NewsStore.isArticleLockedIn(article) && betterdiscord.Data.load("lockingInArticles") && BdApi.React.createElement(
...
betterdiscord.Webpack.getByStrings("USER_ACTIVITY_PLAY", "spotifyData", { searchExports: true });
betterdiscord.Webpack.getByStrings("USER_ACTIVITY_SYNC", "spotifyData", { searchExports: true });
betterdiscord.Webpack.getByStrings("LISTENING", "SPOTIFY).name", { searchExports: true });
betterdiscord.Webpack.getByStrings("M12.7609503,7.08043507", { searchExports: true });
betterdiscord.Webpack.getByStrings("0Zm-2.77-.2-3.33-2.5a.25", "className", { searchExports: true });
...
function AvatarWithPopoutWrapper({ className, user, status, size }) {
	...
	return BdApi.React.createElement(
		Common.Popout,
		{
			...
			clickTrap: true,
			onRequestClose: () => setShowPopout(false),
			renderPopout: () => BdApi.React.createElement(Common.UserProfileWrapperComponent, { currentUser: UserStore.getCurrentUser(), user }),
			position: "right",
			shouldShow: showPopout
		},
		(props) => BdApi.React.createElement(
	...
	return BdApi.React.createElement("div", { className: NowPlayingClasses.partyList }, playerFill.splice(0, 10).map(
		(player) => {
			switch (player) {
				...
				default:
					return BdApi.React.createElement(Tooltip, { note: player.globalName || player.username }, BdApi.React.createElement("div", null, BdApi.React.createElement(
...
function RegularCardTrailing({ activity, user, server, players, v2Enabled }) {
	...
	return BdApi.React.createElement(BdApi.React.Fragment, null, server && BdApi.React.createElement(
	...
	), activityProperties.platform === "SPOTIFY" && BdApi.React.createElement("div", { className: `${NowPlayingClasses.serviceButtonWrapper}` }), activityProperties.platform !== "YT_MUSIC" && activity?.assets ? null : BdApi.React.createElement(
		"div",
		{
		...
		v2Enabled && activity?.party && activity?.party?.size ? null : BdApi.React.createElement(ActivityButtons, { user, activity, onAction: action })
	));
}
function RichCardTrailing({ activity, user, v2Enabled }) {
	...
	const action = Common.ActivityActions({ display: "live", user, activity });
	if (width <= 1240 && width >= 1200) return;
	return BdApi.React.createElement(BdApi.React.Fragment, null, activityProperties.platform !== "YT_MUSIC" && BdApi.React.createElement(
		"div",
		{
			className: `${MainClasses.button} ${NowPlayingClasses.actionsActivity} ${Common.ButtonVoidClasses.lookFilled} ${Common.PositionClasses.flex} ${Common.PositionClasses.noWrap} ${Common.PositionClasses.justifyStart}`,
			style: { flex: "0 1 auto", flexDirection: "column", alignItems: "flex-end", marginLeft: "20px" }
...
function ActivityCardContextMenu({ currentActivity, currentGame }) {
	switch (currentActivity.type) {
		case 0: {
			...
			return BdApi.React.createElement(betterdiscord.ContextMenu.Menu, { navId: "activity-context", onClose: (e) => Common.FluxDispatcher.dispatch({ type: "CONTEXT_MENU_CLOSE" }).finally(e) }, BdApi.React.createElement(betterdiscord.ContextMenu.Item, { id: "open-game-profile", label: "Open Game Profile", action: useGameProfile, disabled: !useGameProfile }), BdApi.React.createElement(
				betterdiscord.ContextMenu.CheckboxItem,
				{
					...
					action: isFollowed || isWhitelisted ? () => {
						NewsStore.blacklistGame(application ?? { id: currentActivity?.application_id });
						isFollowed ? updateFollowStatus(followedGames.filter((item) => item.applicationId !== (application.id ?? currentActivity?.application_id))) : updateBlacklist(blacklist.filter((item) => item.applicationId !== (application.id ?? currentActivity?.application_id)));
					} : () => {
...
function ActivityCardWrapper({ user, activities, voice, streams, v2Enabled }) {
	...
	return activities.map((activity) => {
		...
		return BdApi.React.createElement(ActivityCard, { user, activities, activityProperties, currentActivity, currentGame, players, server, v2Enabled });
	});
}
...
function StreamContextMenu({ stream }) {
	return BdApi.React.createElement(betterdiscord.ContextMenu.Menu, { navId: "watch-stream-context", onClose: (e) => Common.FluxDispatcher.dispatch({ type: "CONTEXT_MENU_CLOSE" }).finally(e) }, BdApi.React.createElement(betterdiscord.ContextMenu.Item, { id: "watch-stream", label: Common.intl.intl.formatToPlainString(Common.intl.t["7Xq/nV"]), action: () => {
...
function MessageButton({ user }) {
	return BdApi.React.createElement(
		...
		Common.intl.intl.formatToPlainString(Common.intl.t["zROXEV"])
	);
}
...
function HeaderActions$1({ card, user }) {
	const [showPopout, setShowPopout] = react.useState(false);
	const refDOM = react.useRef(null);
	return BdApi.React.createElement("div", { className: `${NowPlayingClasses.headerActions} ${Common.PositionClasses.flex} ${Common.PositionClasses.noWrap} ${Common.PositionClasses.justifyStart} ${Common.PositionClasses.alignCenter}`, style: { flex: "0" }, "aria-expanded": showPopout }, BdApi.React.createElement(MessageButton, { user }), BdApi.React.createElement(
		Common.Popout,
		{
			targetElementRef: refDOM,
			clickTrap: true,
			onRequestClose: () => setShowPopout(false),
			renderPopout: () => BdApi.React.createElement(Common.CardPopout, { party: card.party, close: () => setShowPopout(false) }),
			position: "left",
			shouldShow: showPopout
		},
		(props) => BdApi.React.createElement(
...
function HeaderActions({ game }) {
	return BdApi.React.createElement("div", { className: `${NowPlayingClasses.headerActions} ${Common.PositionClasses.flex} ${Common.PositionClasses.noWrap} ${Common.PositionClasses.justifyStart} ${Common.PositionClasses.alignCenter}`, style: { flex: "0" } }, BdApi.React.createElement(FollowButton, { application: game }));
}
...
const LastPlayedStore = () => {
	...
	function fetchLastPlayed() {
		...
		betterdiscord.Data.save("gameIds", gameIds);
...
function IntroCoachmark({ close }) {
	return BdApi.React.createElement("div", { className: `${CoachmarkClasses.coachmark} ${Common.PopoverClasses.popover}` }, BdApi.React.createElement("div", { className: Common.PopoverClasses.graphic }, BdApi.React.createElement("img", { className: CoachmarkClasses.image, alt: "", draggable: "false", src: "https://static.discord.com/assets/de14fab6de78b0fc2f679eb74b735151.svg" })), BdApi.React.createElement("div", { className: CoachmarkClasses.body }, BdApi.React.createElement("div", { className: CoachmarkClasses.bodyHeader }, BdApi.React.createElement("div", { className: CoachmarkClasses.title }, "Activity Feed")), BdApi.React.createElement("div", { className: CoachmarkClasses.bodyContent }, BdApi.React.createElement("div", { className: CoachmarkClasses.content }, "You can customize which games appear on the Activity Feed and other fun toggles in settings. Look for the tab!"))), BdApi.React.createElement("div", { className: CoachmarkClasses.actions }, BdApi.React.createElement("button", { className: `${Common.ButtonManaClasses.button} ${Common.ButtonManaClasses.sm} ${Common.ButtonManaClasses.primary} ${CoachmarkClasses.primaryButton}`, type: "button", onClick: () => {
		ActivityFeedSettingsCoachmarkStore$1.setHasDismissedSettingsCoachmark(true);
...
function IntroCoachmarkPopout({ button }) {
	const [showPopout, setShowPopout] = react.useState(false);
	const isShouldShow = betterdiscord.Hooks.useStateFromStores(ActivityFeedSettingsCoachmarkStore$1, () => ActivityFeedSettingsCoachmarkStore$1.hasDismissedSettingsCoachmark);
	const refDOM = react.useRef(null);
	react.useEffect(() => {
...
const modules_a52d5642 = {
	...
	"unhideBlacklisted": "unhideBlacklisted__97b5e",
...
function FollowedGameItemBuilder({ game, blacklist, updateBlacklist }) {
	...
	return BdApi.React.createElement("div", { className: SettingsClasses.blacklistItem, style: { display: "flex" } }, shouldFallback ? BdApi.React.createElement(FallbackAsset, { className: SettingsClasses.blacklistItemIcon }) : BdApi.React.createElement(
		"img",
		{
					...
					Common.ModalRoot.Modal,
					{
						...
						actions: [
							...
							{ text: "Yes", fullWidth: 1, onClick: () => {
								...
								updateBlacklist(blacklist.filter((item) => item.applicationId !== game.applicationId));
								props.onClose();
							} }
						]
					},
		...
		"button",
		{
			...
			onClick: () => ModalSystem.openModal(
				(props) => BdApi.React.createElement(
					...
					BdApi.React.createElement(BdApi.React.Fragment, null, BdApi.React.createElement("div", { className: MainClasses.emptyText }, "Do you want to unfollow this game? Its announcements will be removed from your Activity Feed."), BdApi.React.createElement("div", { className: MainClasses.emptyText, style: { fontWeight: 600 } }, "This action will require you to restart Discord in order to see changes."))
				)
			)
		},
		"Unfollow"
...
function FollowedGameListBuilder() {
	...
	if (!allGames || !allGames.length) return BdApi.React.createElement(FollowedGameEmptyBuilder, null);
	const filtered = react.useMemo(() => {
		const _query = query.toLowerCase();
		return allGames?.filter((item) => item?.name?.toLowerCase().includes(_query));
	}, [allGames, query]);
	return BdApi.React.createElement(BdApi.React.Fragment, null, BdApi.React.createElement(betterdiscord.Components.SearchInput, { className: SettingsClasses.search, onChange: (e) => setQuery(e.target.value.toLowerCase()), placeholder: "Search for Games" }), filtered?.length ? BdApi.React.createElement("div", { className: SettingsClasses.blacklist }, filtered.sort((a, b) => a.name.localeCompare(b.name)).map(
...
function CoachmarkWrapper({ button }) {
	if (useSelectedState() && !ActivityFeedSettingsCoachmarkStore$1.hasDismissedSettingsCoachmark) {
