...
function ExternalItemBuilder({service}) {
    ...
    return (
        <div className={SettingsClasses.itemContainer} style={{ display: "flex" }}>
            <item.icon className={SettingsClasses.itemIcon} color="WHITE" style={{ backgroundColor: item.color, padding: "5px" }} />
            <div className={SettingsClasses.itemTextContainer}>
                <div className={SettingsClasses.itemName}>{item.name || "Unknown Source"}</div>
    ...
    return (
        <div className={SettingsClasses.container}>{
            Object.keys(settings.external).map((key) => {
                return (
                    <>
                        <ExternalItemBuilder service={key} key={key} />
